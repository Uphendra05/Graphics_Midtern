...
void SoftBodyObjs::UpdateMeshVertices()
{
	for (Point* point : m_ListOfPoints)
	{
		...
		point->vertexData->Position = glm::vec3(point->curPosition.x, point->curPosition.y, point->curPosition.z);
...
void SoftBodyObjs::CalculateMatrixForVertices(std::shared_ptr<Mesh> vertSize) const
{
	...
	{
		glm::vec4 vertexMatrix = glm::vec4(vertSize->vertices[index].Position.x,
			vertSize->vertices[index].Position.y,
			vertSize->vertices[index].Position.z, 1.0f);

		vertexMatrix = transform.GetModelMatrix() * vertexMatrix;


		vertSize->vertices[index].Position.x = vertexMatrix.x;
		vertSize->vertices[index].Position.y = vertexMatrix.y;
		vertSize->vertices[index].Position.z = vertexMatrix.z;
